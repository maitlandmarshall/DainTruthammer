...
def _candidate_character_dirs(repo_root: Path, world_dir: Path | None, codex_root: str) -> list[Path]:
	dirs: list[Path] = []
	if world_dir:
		dirs.append(world_dir / "lore" / "characters")
		dirs.append(world_dir / "characters")
	dirs.append(repo_root / codex_root / "Characters")
	dirs.append(repo_root / codex_root.lower() / "Characters")
	dirs.append(repo_root / codex_root.lower() / "characters")
...
def _resolve_character_refs(repo_root: Path, world_dir: Path | None, codex_root: str, character_name: str) -> list[Path]:
	"""
	...
	Supports both `codex/worlds/<World>/lore/characters` and this repo's
	`Codex/Characters` layout.
	"""
	slug = _slugify(character_name)
	space_name = character_name.replace("_", " ").strip()

	images: list[Path] = []
	for characters_dir in _candidate_character_dirs(repo_root, world_dir, codex_root):
		entry_images: list[Path] = []
		entry_candidates = [
			characters_dir / f"{character_name}.md",
			characters_dir / f"{space_name}.md",
			characters_dir / f"{slug}.md",
			characters_dir / f"{slug.replace('_', ' ')}.md",
		]
		for entry in entry_candidates:
			if entry.exists():
				entry_images.extend(_resolve_images_from_markdown(entry))
		if entry_images:
		...
		folder_candidates = [
			...
			characters_dir / space_name,
			characters_dir / slug,
			characters_dir / slug.replace("_", " "),
		]
		common_names = [
			"portrait.png",
			"portrait.jpg",
		...
		direct_candidates = [
			...
			characters_dir / f"{space_name}.png",
			characters_dir / f"{space_name}.jpg",
			characters_dir / f"{slug}.png",
			characters_dir / f"{slug}.jpg",
			characters_dir / f"{slug}_portrait.png",
			characters_dir / f"{slug}_portrait.jpg",
		]
...
def main() -> None:
	...
	parser.add_argument("--world", default=None, help="Optional world folder name under codex/worlds/")
	parser.add_argument("--codex-root", default="Codex", help="Repo codex root for local campaign layout")
	parser.add_argument("--prompt", required=True)
	parser.add_argument("--out", required=True)
	parser.add_argument("--character", action="append", default=[], help="Character name to resolve as reference")
	parser.add_argument("--input-image", action="append", default=[], help="Explicit reference image path")
	...
	parser.add_argument("--retry-delay", default=None)
	parser.add_argument("--dry-run", action="store_true", help="Print resolved references and command without calling the API")
	...
	for c in args.character:
		refs = _resolve_character_refs(repo_root, world_dir, args.codex_root, c)[: args.max_refs_per_character]
	...
	if args.dry_run:
		print(json.dumps({
			"command": cmd,
			"references": ref_map,
			"explicit_references": [str(Path(p).resolve()) for p in args.input_image],
			"all_reference_images": [str(p) for p in ref_images],
		}))
		return
